...
def tabs_validate(file, sheets):
    ...
    for tab in required_sheets.keys():
        if not any(_canonical_sheet_name(k) == tab for k in sheets.keys()):
...
def _reset_combine_cache():
    combine_data_sheets.cache = {}
...
def combine_data_sheets(sheets_dict, sheet, debug=False):
    ...
    target = _canonical_sheet_name(sheet)

    # Iterate through all the sheets in the dictionary
    for sheet_name, df in sheets_dict.items():
        logical = sheet_name.split('@', 1)[0]
        if _canonical_sheet_name(logical) != target:
            continue
        if debug:
            print("    combineDataSheets() " + sheet + " in " + sheet_name)
        if df.empty:
            part = df.copy()
        else:
            part = df.dropna(axis=1, how='all')
        if combined_df.empty:
            combined_df = part.copy()
        else:
            combined_df = pd.concat([combined_df, part], ignore_index=True)
...
def _canonical_sheet_name(name):
    if not isinstance(name, str):
        return name
    key = name.strip()
    return _SHEET_CANONICAL.get(key.lower(), key)
...
def _ensure_vmultipath_columns(df):
    """
    RVTools uses Display name; some exports use Disk Name or omit the column.
    """
    if df is None or len(getattr(df, 'columns', [])) == 0:
        return df
    if 'Display_name' in df.columns:
        return df
    for src in ('Disk_Name', 'Disk_name', 'DisplayName', 'Display'):
        if src in df.columns:
            return df.rename(columns={src: 'Display_name'})
    if 'Disk' in df.columns:
        out = df.copy()
        out['Display_name'] = out['Disk'].astype(str)
        return out
    return df
...
def clean_and_fix_data(sheets_dict):
    ...
    vMultiPath_df = _ensure_vmultipath_columns(combine_data_sheets(sheets_dict, 'vMultiPath'))
    if 'Display_name' in vMultiPath_df.columns:
